...
sliderSlick.init({
    ...
    cssEase: 'linear',
    focusOnSelect: true,
    prevArrow :   '<button type="button" class="slick-prev pe-7s-angle-left"></button>',
    nextArrow :   '<button type="button" class="slick-next pe-7s-angle-right"></button>',
});
sliderSlick.run();
...
$(function(){
    // carousel init 
    const  carousel = new OwlCarousel("#test");
    carousel.responsive = {0:{items: 1}, 480: {items: 3}, 992: {items: 4} }
    carousel.init()

    // trigger control carousel 
    var owl = $('.owl-carousel');
    owl.owlCarousel();

    // Go to the next item
    $('#btn-next-custom').click(function() {
        owl.trigger('next.owl.carousel', [300]);
    });

    // Go to the previous item
    $('#btn-pre-custom').click(function() {
...
creativeCarousel.responsive = {0: {items: 1.5}, 768: {items: 3}, 1200: {items: 7.5 }};
...
customerCarousel.responsive = {0: {items: 2}, 768: {items: 2}, 1200: {items: 5 }};
...
$('.nav-list .nav-item-main .nav-item-sub').click(function(){
    $('.nav-menu .nav-list .nav-item-main .item-sub .sub_dropdown-lv2').toggleClass('show');
})
...
$("#mobile-menu .hassubmenu").on("click", function(event) {
    event.preventDefault();
  if ($(this).hasClass("active")) {
    $(this).removeClass("active"); 
  } else {
    
    $("#mobile-menu .hassubmenu").removeClass("active");
